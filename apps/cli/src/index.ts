...
function parseArgs(argv: string[]) {
  const args: { command?: string; scope?: string; target?: string } = {};
  args.command = argv[0];
...
async function promptScope(): Promise<"repo" | "global"> {
  ...
  if (answer === "1") return "repo";
  console.error(`Opção inválida: "${answer}"`);
  process.exit(1);
}

async function runInit(args: ReturnType<typeof parseArgs>) {
